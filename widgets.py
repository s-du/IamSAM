...
class PhotoViewer(QGraphicsView):
    ...
    def mousePressEvent(self, event):
        if self.select_point_plus:
            self._current_point = self.mapToScene(event.pos())
            self.pluspoint_count += 1
            self.list_point_plus.append([int(self._current_point.x()), int(self._current_point.y())])
            self.draw_ellipse(self._current_point, 'plus')
            self.select_point_plus = False
            self.end_pluspoint_selection.emit()
        elif self.select_point_min:
            self._current_point = self.mapToScene(event.pos())
            self.minpoint_count += 1
            self.list_point_min.append([int(self._current_point.x()), int(self._current_point.y())])
            self.draw_ellipse(self._current_point, 'min')
            self.select_point_min = False
            self.end_minpoint_selection.emit()
